...
public class carDrive : MonoBehaviour
{
    ...
    public float torque = 150;              //rotationalforce
    public float maxSteerAngle = 30;        //steering angle
    public float maxBrakeTorque = 500;         //Brake force
    ...
    int currentGear = 1;
    float currentGearPer;           //gear percentage
    ...
    public void CalcEngineSound()
    {
        ...
        currentGearPer = Mathf.Lerp(currentGearPer, targetGearFactor, Time.deltaTime * 5f);

        var gearNumFactor = currentGear / (float) numGears;
        rpm = Mathf.Lerp(gearNumFactor, 1, currentGearPer);
